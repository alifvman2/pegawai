...
<head>
	...
	<title>Edit Pegawai {{ $data->name }}</title>
	...
	<div class="container">
		<div class="card p-5">
			<div class="card-header">
				Edit Pegawai {{ $data->name }}
			</div>
			<div class="card p-3">
				<form action="{{ route('update_peg') }}" method="post">
					@CSRF
					...
					<div class="control-group">
						...
						<select class="form-control" name="kelurahan" id="kelurahan">
							...
							@foreach($kelurahan as $kel)
								<option value="{{ $kel->id }}" @if($data->id_kel == $kel->id) selected @endif>{{ $kel->name }}</option>
							@endforeach
						</select>
					</div>
